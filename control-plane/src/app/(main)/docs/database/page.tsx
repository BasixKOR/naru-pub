...
export default function DatabaseDocs() {
          ...
          <article className="min-w-0 space-y-12 leading-8 [&_a]:underline [&_a]:underline-offset-4">
            <Section id="start" title="01 · 빠른 시작">
              <p>
                ...
                만들고 읽기를 공개로 바꾼 뒤, 페이지에 아래 스크립트를 넣으세요.
  ...
  for (const post of documents) console.log(post.id, post.data);
</script>`}</Code>
              <p>
            ...
            <Section id="read" title="03 · 읽기">
              <Code>{`const posts = naru.public.collection("posts");
...
try {
  ...
  alert("다른 곳에서 먼저 저장했습니다. 새로고침 후 다시 시도하세요.");
            ...
            <Section id="owner" title="05 · 관리자 로그인">
              <p>
...
loginButton.onclick = () =>
  naru.auth.signIn({ collections: ["posts", "drafts"] });

if (owner) {
  await owner.collection("posts").set("hello", { title, body });
  await owner.signOut();
}`}</Code>
              <p>
                권한은 기본 24시간 뒤 만료되며, 제어판에서 페이지마다 줄이거나
                취소할 수 있습니다. 만료되면 요청이 <code>AUTH_REQUIRED</code>로
                실패하니 다시 로그인하세요. 관리자 페이지에는 신뢰하는
                스크립트만 넣으세요.
              </p>
              <h3 className="font-bold">트랜잭션</h3>
              <p>
                <code>owner.transaction()</code>은 여러 컬렉션의{" "}
                <code>set</code>·<code>delete</code>를 최대 100개까지 묶어, 모두
                반영하거나 하나도 반영하지 않습니다.
              </p>
  ...
  .list({
    sort: [["date", "desc"]],
                  ...
                  },
                  {
                    ...
                    code: `naru.public.collection("guestbook")
  .add({ name, message });`,
                  },
                  {
                    title: "비공개 문의 받기",
                    setup: "inquiries · 읽기 관리자만 / 쓰기 누구나 생성만",
                    code: `naru.public.collection("inquiries")
  .add({ email, message });`,
                  },
                  {
                    title: "초안 저장 후 공개",
                    setup: "drafts · 읽기 관리자만 / 쓰기 관리자만",
                    code: `owner.transaction([
  { collection: "posts",
    set: { id, data } },
  { collection: "drafts",
    delete: { id } },
  ...
  .list({
    filter: { postId: "hello" },
            ...
            <Section id="errors" title="07 · 한도와 오류">
              ...
              />
              <Code>{`try {
  await naru.public.collection("guestbook").add({ message });
} catch (error) {
  status.textContent =
